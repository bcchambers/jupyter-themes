...
def install_precompiled_theme(theme):
    ...
    if os.path.isdir(compiled_dir_user) and "{}.css".format(theme) in os.listdir(
        compiled_dir_user
    ):
...
def style_layout(
    ...
    if os.path.isdir(styles_dir_user) and "{}.less".format(theme) in os.listdir(
        styles_dir_user
    ):
    ...
    tcPromptWidth = promptMinWidth + 0.5
    ...
    if altprompt:
        promptPadding = ".1em"
        promptMinWidth = 8
        outpromptMinWidth = promptMinWidth + 0.5
        tcPromptWidth = promptMinWidth + 0.5
...
def toggle_settings(toolbar=False, nbname=False, hideprompt=False, kernellogo=False):
    ...
    if nbname:
        toggle += (
            "span.save_widget span.filename {margin-left: 8px; height: initial;"
            "font-size: 100%; color: @nb-name-fg; background-color:"
            "@cc-input-bg;}\n"
        )
        toggle += (
            "span.save_widget span.filename:hover {color:"
